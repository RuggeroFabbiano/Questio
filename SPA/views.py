...
class AnswerView(generics.CreateAPIView):
    ...
    def perform_create(self, serializer):
        relatedQuestion = get_object_or_404(Question, slug=self.kwargs.get('slug'))
        if relatedQuestion.answers.filter(author=self.request.user).exists():
            raise ValidationError("You've already answered this question")
        serializer.save(author=self.request.user, question=relatedQuestion)

class QuestionAnswersView(generics.ListAPIView):
    serializer_class = AnswerSerializer
    permission_classes = [IsAuthenticated, ReadOnlyOrIsAuthor]
    def get_queryset(self):
        return Answer.objects.filter(question__slug=self.kwargs.get('slug')).order_by('-creationDate')
